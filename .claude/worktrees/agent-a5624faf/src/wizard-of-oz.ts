...
export function detectObservations(
  ...
  // During planning phase, don't fire generic greetings — the orchestrator
  // already triggered a focused planning prompt via the LLM. Only fire
  // planning-specific nudges if the user hasn't responded after agents asked.
  if (phase === 'planning') {
    ...
    // If agents asked a question but user hasn't replied after a few messages
  ...
  // Skip empty-doc greeting if orchestrator already handles it via planning phase
  // Only show if we're in active phase and the doc is sparse but has some content
  if (plain.trim().length < 30 && messages.length <= 2) {
    // During active phase with an empty doc, this means the user already
    // gave direction — don't re-greet
    if (phase === 'active') {
