...
describe('extractDocStructure', () => {
  ...
  it('computes totalWords and avgSectionWords', () => {
    const doc = '## A\none two three\n## B\nfour five six seven eight'
    const result = extractDocStructure(doc)
    expect(result.totalWords).toBe(8)
    expect(result.avgSectionWords).toBe(4)
  })

  it('detects thin sections relative to average', () => {
    const doc = '## Big\none two three four five six seven eight nine ten eleven twelve thirteen fourteen fifteen sixteen seventeen eighteen nineteen twenty\n## Tiny\nonly two\n## Medium\none two three four five six seven eight nine ten'
    const result = extractDocStructure(doc)
    expect(result.thinSections).toContain('Tiny')
    expect(result.thinSections).not.toContain('Big')
  })

  it('detects empty sections', () => {
    const doc = '## Filled\nsome content here\n## Empty\n## Also Empty\n## Another Filled\nmore content'
    const result = extractDocStructure(doc)
    expect(result.emptySections).toContain('Empty')
    expect(result.emptySections).toContain('Also Empty')
    expect(result.emptySections).not.toContain('Filled')
  })

  it('tracks heading levels', () => {
    const doc = '# Title\ntext\n## Section\ntext\n### Subsection\ntext'
    const result = extractDocStructure(doc)
    expect(result.headingLevels['Title']).toBe(1)
    expect(result.headingLevels['Section']).toBe(2)
    expect(result.headingLevels['Subsection']).toBe(3)
  })

  it('detects intro content before first heading', () => {
    const doc = 'This is an intro paragraph.\n## First Section\ncontent'
    const result = extractDocStructure(doc)
    expect(result.hasIntro).toBe(true)
    expect(result.introWords).toBeGreaterThan(0)
  })

  it('reports no intro when doc starts with heading', () => {
    const doc = '## First Section\ncontent here'
    const result = extractDocStructure(doc)
    expect(result.hasIntro).toBe(false)
    expect(result.introWords).toBe(0)
  })
})
