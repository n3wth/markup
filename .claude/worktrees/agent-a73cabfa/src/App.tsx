...
function App() {
  ...
  const resizingRef = useRef<'chat' | null>(null)
  ...
  // Panel resize handlers (chat only -- sidebar is fixed width from icon bar)
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      if (!resizingRef.current) return
      e.preventDefault()
      // Chat is on the left, so width = cursor X minus offset (icon bar + sidebar panel)
      const offset = 52 + (sidebarExpanded ? 240 : 0)
  ...
  return (
    <div className={`app-shell ${activeSession ? 'app-shell-active' : ''}`}>
      ...
      <IconBar
        ...
        onHome={() => { resetToHome(); setSidebarExpanded(false) }}
        user={user ?? null}
      ...
      {showCommandPalette && (
        <CommandPalette
          ...
          commands={[
            ...
            { id: 'toggle-sidebar', label: sidebarExpanded ? 'Hide sidebar' : 'Show sidebar', action: () => setSidebarExpanded(v => !v) },
            ...(activeSession ? [
